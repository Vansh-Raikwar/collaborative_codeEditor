...
function FileContextProvider({ children }) {
    ...
    const createDirectory = useCallback(
        (
            ...
            let resolvedParentId = parentDirId || fileStructure.id
            const parentDir = findParentDirectory(fileStructure, resolvedParentId)
            if (!parentDir) {
                const findParent = (curr) => {
                    if (curr.children) {
                        for (const child of curr.children) {
                            if (child.id === resolvedParentId) return curr
                ...
                const actualParent = findParent(fileStructure) || fileStructure
                resolvedParentId = actualParent.id
            }

            const addDirectoryToParent = (
                directory,
            ) => {
                if (directory.id === resolvedParentId) {
    ...
    const createFile = useCallback(
        (
            ...
            let resolvedParentId = parentDirId || fileStructure.id
            let parentDir = findParentDirectory(fileStructure, resolvedParentId)
            if (!parentDir) {
                const findParent = (curr) => {
                    if (curr.children) {
                        for (const child of curr.children) {
                ...
                parentDir = findParent(fileStructure) || fileStructure
                resolvedParentId = parentDir.id
            }
